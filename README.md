...
GitHub에서 해당 파일을 열고 연필 버튼으로 수정한 뒤 Commit changes를 누르면 홈페이지가 갱신됩니다. CV·슬라이드를 같은 경로와 파일명으로 교체하면 기존 링크가 새 파일로 연결됩니다. ACM 270 강의록 PDF와 다운로드 링크는 초안 수정 중으로 잠시 내려두었습니다. 완성본이 준비되면 PDF를 다시 게시하고 홈페이지의 강의 자료 링크를 복원합니다. 홈페이지 본문의 내용과 원고 상태는 `_pages/about.html`에서도 따로 수정해야 합니다.
...
홈페이지 내용은 제공된 최신 CV와 본인의 수정 요청을 반영했습니다. CV·SIAM UQ 슬라이드는 공개 게시가 승인된 원본 파일입니다. ACM 270 강의록 두 개는 2026년 9월 14일에 공개를 잠시 중단했습니다. 홈페이지의 ACM 270 날짜는 실제 강의일인 2026년 4월 28일과 30일이며, PDF 표지 날짜와는 다릅니다. 사진·Google Scholar 링크·학부 소개는 기존 개인 홈페이지에서 가져왔습니다. 새 디자인은 Lin Lin, Simran Arora, Sinho Chewi의 학술 홈페이지 구성을 참고해 작성했습니다. 이전 테마의 라이선스와 관련 소스는 그대로 남아 있습니다.
